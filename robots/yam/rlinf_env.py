...
class YamAgentEnv:
    ...
    def __init__(
        ...
        *,
        ...
        configured_hz = self.config.get("control_hz", MODEL_SPEC.control_hz)
        if int(configured_hz) != int(MODEL_SPEC.control_hz):
            raise ValueError(
                f"YAM control_hz is fixed by MODEL_SPEC at {MODEL_SPEC.control_hz}"
            )
    ...
    def _begin_episode(
        ...
        *,
        ...
        self._episode_id = str(episode_id or uuid.uuid4().hex)
    ...
    def reset_operator_receipt(self, *, receipt: dict[str, Any]) -> dict[str, Any]:
        """Consume a ready receipt from the local operator channel."""
        ready = self._validate_operator_receipt(receipt, event="ready")
        self._operator_ready_receipt = ready
        return dict(ready)

    def record_operator_success(
        self,
        receipt: dict[str, Any],
    ) -> dict[str, Any]:
        """Consume a success receipt from the local operator channel."""
        if receipt.get("event") not in {"success", "failure", "abort"}:
            raise ValueError(
                "operator verdict receipt event must be success/failure/abort"
            )
        self._consume_terminal_receipt(receipt)
        return dict(self._operator_success_receipt)

    def operator_receipt_requests(self) -> dict[str, Any]:
        """Return the current receipt target for the operator CLI."""
        return {
            "episode_id": self._episode_id,
            "actual_seed": int(self._actual_seed),
            "operator_receipt_path": self.operator_receipt_path,
            "valid_events": ["ready", "success", "failure", "abort"],
            "ready_required_before_motion": True,
            "request_id": "generated by robots.yam.operator_control.write_receipt",
        }
    ...
    def _build_runtime(self) -> Any:
        env_config_cls = import_rlinf_object(
            "rlinf.envs.realworld.yam.config", "DualYamJointEnvConfig", self.config
        )
        runtime_cls = import_rlinf_object(
            "rlinf.envs.realworld.yam.control_runtime", "YamControlRuntime", self.config
        )
        factory_cls = import_rlinf_object(
            "rlinf.envs.realworld.yam.i2rt_backend",
            "I2RTYamBackendFactory",
            self.config,
        )
        runtime_config = env_config_cls(
            is_dummy=False,
            task_description=self.task_language,
            step_frequency=self.control_hz,
            max_num_steps=self.step_lim,
            ...
            camera_warmup_timeout_s=float(
                self.config.get("camera_warmup_timeout_s", 10.0)
            ),
            camera_frame_timeout_s=float(
                self.config.get("camera_frame_timeout_s", 1.0)
            ),
            camera_stale_timeout_s=float(
                self.config.get("camera_stale_timeout_s", 2.0)
            ),
        ...
        hardware = SimpleNamespace(
            ...
            left_leader=self._device_config("left_leader", "can_lead_l", leader=True),
            right_leader=self._device_config("right_leader", "can_lead_r", leader=True),
            cameras=[],
        )
        return runtime_cls(runtime_config, hardware, factory_cls())

    def _device_config(
        self, role: str, default_channel: str, *, leader: bool = False
    ) -> SimpleNamespace:
        ...
        return SimpleNamespace(
            ...
            gripper_type=str(
                values.get(
                    "gripper_type",
                    "yam_teaching_handle" if leader else "flexible_4310",
                )
            ),
            ee_mass=values.get("ee_mass"),
            gripper_limits=values.get("gripper_limits", [0.0, 1.0] if leader else None),
            gravity_comp_factor=values.get(
                "gravity_comp_factor",
                None if leader else [1.0, 1.1, 1.1, 1.2, 1.0, 1.0],
            ),
            grav_comp_kd=values.get("grav_comp_kd"),
            coulomb_friction=values.get("coulomb_friction"),
            use_coulomb_friction=bool(values.get("use_coulomb_friction", not leader)),
            bilateral_kp=float(values.get("bilateral_kp", 0.0)),
            gripper_invert=bool(values.get("gripper_invert", False)),
    ...
    def _capture_observation_once_locked(
        ...
        snapshot_api_supports_freshness = True
        snapshot_api_error = None
        try:
            ...
            snapshot_api_supports_freshness = False
            snapshot_api_error = str(error)
            snapshot = self._cameras.snapshot()
        ...
        for name, frame in snapshot["views"].items():
            ...
            frame_bracketed = (
                snapshot_api_supports_freshness
                and host_before_monotonic_s is not None
            ...
            elif name != "top" and not snapshot_api_supports_freshness:
                projection_limitation = (
                    "camera snapshot API cannot wait for a frame after qpos sample: "
                    + str(snapshot_api_error)
                )
            ...
            camera_meta.update({
                ...
                "frame_bracketed_by_qpos_samples": bool(frame_bracketed),
                "fk_timestamp_assumption": "host_time_bounded_stationary_approx_not_exposure_sync",
                "qpos_before_snapshot": qpos_before.copy(),
                "qpos_after_snapshot": qpos_after.copy(),
                "qpos_delta_before_after_snapshot_l2": qpos_delta_l2,
                "qpos_delta_before_after_snapshot_linf": qpos_delta_linf,
                "qpos_static_tolerance_rad": qpos_static_tolerance,
            })
            views[name] = {
                "rgb": frame.rgb.copy(),
        ...
        obs = {
            "frames": {name: views[name]["rgb"] for name in CAMERA_NAMES},
        ...
        robot_state.update({
            "qpos_before_capture": qpos_before,
            "qpos_after_capture": qpos_after,
            "qpos_before_snapshot_timestamp_s": timestamps_before,
            "qpos_after_snapshot_timestamp_s": timestamps_after,
            "qpos_timestamp_s": timestamps_after,
            "qpos_delta_before_after_snapshot_l2": qpos_delta_l2,
            "qpos_delta_before_after_snapshot_linf": qpos_delta_linf,
            "capture_host_interval_s": snapshot.get("capture_host_interval_s"),
            "camera_sync_mode": snapshot.get("sync_mode"),
            "projection_sync_limitation": (
                "host timestamps bound frame arrival/capture calls only; this is "
                "a stationary approximation, not hardware exposure synchronization"
            ),
        })
        info = {
    ...
    def _episode_status(self) -> dict[str, Any]:
        return {
            ...
            "operator_receipt_requests": self.operator_receipt_requests(),
    ...
    def _consume_ready_receipt_locked(self) -> dict[str, Any] | None:
        ...
        if key in self._consumed_receipt_ids:
            return None
        return self.reset_operator_receipt(receipt=receipt)
