...
from robots.behavior.mcp_server import HttpMcpServer
...
class CodexPlanner:
    ...
    def solve(
        ...
        *,
        ...
        def _capture_and_interrupt(tool_result: Any) -> None:
            if recorder.capture_tool_result(tool_result):
                logger.info(
                    "terminal tool result returned; interrupting active Codex turn"
                )
                _interrupt_active_turn(state)

        mcp_server = HttpMcpServer(
            toolkit,
            on_tool_result=_capture_and_interrupt,
        )
        ...
        try:
            ...
            elif "error" in state and recorder.finish_result is None:
                exc = state["error"]
        ...
        if recorder.finish_result is not None:
            # turn.interrupt() is the expected control path after a verified
            # terminal tool result, not a planner failure.
            error = None
    ...
    def _run_session(
        ...
        try:
                ...
                with (
                    ...
                    try:
                        for event in turn.stream():
                            ...
                            if recorder.finish_result is not None:
                                logger.info(
                                    "terminal tool result captured: %s",
            ...
            if recorder.finish_result is None:
                state["error"] = e
            else:
                logger.info(
                    "Codex turn stopped after terminal tool result: %s",
                    type(e).__name__,
                )
...
class _Recorder:
    ...
    def capture_tool_result(self, tool_result: Any) -> bool:
        """Capture the first toolkit-verified terminal result exactly once."""

        result = getattr(tool_result, "result", None)
        if (
            not getattr(tool_result, "is_finish", False)
            or not isinstance(result, dict)
            or result.get("_finish") is not True
        ):
            return False
...
def _interrupt_active_turn(state: dict[str, Any]) -> None:
