...
class BehaviorToolkit(Toolkit):
    ...
    def __init__(
        ...
        *,
        ...
        self.enable_stage3_press = bool(enable_stage3_press)
        if control_mode == FULL_TASK_VLA_MODE:
            ...
            if primitives_kwargs is None:
                raise ValueError("pi0_nav_pick_vla mode requires primitives_kwargs")
            self._primitives = BehaviorPrimitives(**primitives_kwargs)
            self.add_tool(
                "pi0_nav_pick",
                PI0_NAV_PICK_SPEC,
            ...
            if self.enable_stage3_press:
                stage3_specs = {
                    "post_pick_close_press_gripper": (
                        POST_PICK_CLOSE_PRESS_GRIPPER_SPEC
                    ),
                    "inspect_toggle_geometry": INSPECT_TOGGLE_GEOMETRY_SPEC,
                    ...
                    "post_success_hold_frames": POST_SUCCESS_HOLD_FRAMES_SPEC,
                }
                if tuple(stage3_specs) != STAGE3_PRESS_TOOL_NAMES:
                    raise RuntimeError("stage-3 press schema order mismatch")
                for name, spec in stage3_specs.items():
                    self.add_tool(
                        name,
                        spec,
                        lambda _name=name, **kwargs: self._primitives.stage3_press_call(
                            _name, **kwargs
                        ),
                    )
    ...
    def write_recipe(self, recipe_tag: str) -> str | None:
        if self.control_mode in {HYBRID_VLM_PI0_MODE, PI0_NAV_PICK_VLA_MODE}:
            ...
            mode_result = {
                ...
                "tool_trace_path": str(self._primitives.output_dir / trace_name),
