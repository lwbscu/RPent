...
never turn this run's right/left assignment into a general rule. A debug mirror
is only an attachment-faithful development resume and never substitutes for the
checkpoint or official task result.
...
final environment step, motion trace, state-2 SHA, and success debug mirror for
review. A later zero consecutive-contact counter does not undo a latched
