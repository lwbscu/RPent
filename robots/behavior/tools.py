...
_SAFE_CHECKPOINT_NAME = re.compile(r"[A-Za-z0-9][A-Za-z0-9_.-]{0,127}")
...
class BehaviorPrimitives:
    ...
    def inspect_post_pick_state(
        ...
        *,
        ...
        if _SAFE_CHECKPOINT_NAME.fullmatch(checkpoint_name) is None:
        ...
        if role == "held":
            ...
            if head_target_uv is not None:
                ...
                if isinstance(radius_px, bool):
                    raise ValueError(
                        "button_goal.head_target_radius_px must be finite"
                    )
            ...
            if alignment_phase == "position_first" and head_target_uv is None:
                raise ValueError(
                    "button_goal.alignment_phase=position_first requires "
                    "head_target_uv"
                )
    ...
    def save_post_pick_robot_state_checkpoint(
        ...
        *,
        ...
        if checkpoint_name != "state_checkpoint_2":
            raise ValueError(
                "post-pick save_robot_state_checkpoint only writes state_checkpoint_2"
            )
        ...
        if visual_review is not True:
            raise ValueError("state_checkpoint_2 requires visual_review=true")
        return self._public_checkpoint_result(
