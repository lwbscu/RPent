...
def generate_button_goal_pose_candidates(
    *,
    ...
    if orientation_goal not in {"side_to_press", "preserve_current"}:
        raise ValueError(
            "orientation_goal must be side_to_press or preserve_current"
        )
    ...
    for position_index, (position_delta, position_magnitude) in enumerate(positions):
        ...
        for blend_index, blend in enumerate(blends):
            ...
            if orientation_goal == "preserve_current":
                ...
                nominal_quat = quaternion_xyzw_from_rotation_matrix(
                    world_radio[:3, :3]
                )
                ...
                side_normal = _side_view_normal(
                    face_to_press, head_axis, button_normal
                )
            ...
            for orientation_index, (
                ...
                target_rotation = pose_matrix_xyzw(
                    [0.0, 0.0, 0.0], target_radio_quat
                )[:3, :3]
                ...
                chest_lateral_drift = float(
                    np.linalg.norm(
                        button_delta
                        - achieved_chest_translation * chest_direction
                    )
                )
    ...
    *,
    ...
    if alignment_phase == "observation":
        if eef_to_camera_transform is None:
            raise ValueError(
                "observation alignment requires eef_to_camera_transform"
            )
    ...
    for index, roll_deg in enumerate(roll_degrees[:budget]):
        ...
        if alignment_phase == "observation":
            ...
            target_quat = quaternion_xyzw_from_rotation_matrix(
                target_press[:3, :3]
            )
        ...
        eligible = (
            ...
            if alignment_phase == "final"
            else bool(camera_center_angle_deg is not None and camera_center_angle_deg <= 1e-6)
        )
        candidates.append(
            {
                "candidate_id": (
...
def evaluate_geometry(
    *,
    ...
    criteria = {
        ...
        "normal_opposition": opposition_angle
        <= float(max_opposition_angle_deg) + 1e-9,
