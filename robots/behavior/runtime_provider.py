...
def start_env_server(
    ...
    *,
    ...
    if getattr(args, "behavior_stage3_press", False):
        command.append("--behavior-stage3-press")
...
class BehaviorRuntimeHandle(RuntimeHandle):
    ...
    def _delete_temporary_checkpoints(self) -> None:
        """Remove run-local temporary restore points at lifecycle end."""

        checkpoint_dir = self.output_dir / "state_checkpoints"
        for path in checkpoint_dir.glob("tmp_state_checkpoint_*.json"):
            if path.is_file() or path.is_symlink():
                path.unlink()
        for path in checkpoint_dir.glob("tmp_state_checkpoint_*.json.tmp"):
            if path.is_file() or path.is_symlink():
                path.unlink()
    ...
    def close(self) -> None:
        ...
        if paused_runtime_path.is_file():
            ...
            preserve_runtime = bool(
                ...
                and not getattr(self.toolkit, "enable_stage3_press", False)
                ...
                and self.toolkit.official_task_success is not True
            )
        lifecycle_path = paused_runtime_path
        lifecycle_status = "paused"
        if (
            not preserve_runtime
            and getattr(self.toolkit, "control_mode", None) == PI0_NAV_PICK_VLA_MODE
            and not getattr(self.toolkit, "enable_stage3_press", False)
        ):
        ...
        if self.manifest is not None:
            try:
                ...
                if failure is None:
                    failure = error
        try:
            self._delete_temporary_checkpoints()
        except BaseException as error:
            if failure is None:
                failure = error
...
class BehaviorRuntimeProvider:
    ...
    def add_cli_args(self, parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument(
            ...
            action="store_true",
            help=(
                "Extend turning_on_radio pi0_nav_pick_vla with the explicitly "
                "authorized stage-3 press surface after state_checkpoint_2."
            ),
        )
    ...
    def validate_args(
        ...
        if getattr(args, "behavior_stage3_press", False) and (
            args.behavior_control_mode != PI0_NAV_PICK_VLA_MODE
            or args.task_name != "turning_on_radio"
        ):
            parser.error(
                "--behavior-stage3-press requires "
                "--behavior-control-mode pi0_nav_pick_vla and "
    ...
    def prompt_vars(
        ...
        *,
        ...
        return {
            ...
            "behavior_stage3_press": getattr(args, "behavior_stage3_press", False),
            **mode_instructions(
                args.behavior_control_mode,
                task_name=args.task_name,
                ...
                enable_stage3_press=getattr(args, "behavior_stage3_press", False),
            ),
        }
    ...
    def _expected_meta(self, args: argparse.Namespace) -> dict[str, Any]:
        return {
            ...
            "seed": args.seed,
            "max_episode_steps": args.max_episode_steps,
            "stage3_press_enabled": getattr(args, "behavior_stage3_press", False),
        }
        ...
        *,
        ...
        try:
            if args.no_driver:
                ...
                if args.behavior_control_mode in VLA_CONTROL_MODES and not vla_endpoint:
