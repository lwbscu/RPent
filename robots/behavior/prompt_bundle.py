...
def _render_prompt(filename: str, **values: object) -> str:
    ...
    for name, value in values.items():
        ...
        if placeholder not in prompt:
            raise ValueError(
                f"missing prompt placeholder {placeholder!r} in {filename}"
            )
...
PLANNER_SYSTEM_INSTRUCTIONS = "\n\n".join([_load_prompt("planner_system.md")])
...
PI0_NAV_PICK_PRESS_RESUME_INSTRUCTIONS = _load_prompt(
    ...
    *,
    ...
    if control_mode == PI0_NAV_PICK_VLA_MODE:
        ...
        if task_name == "turning_on_radio":
            ...
            if enable_stage3_press:
                parts.append(PI0_NAV_PICK_PRESS_RESUME_INSTRUCTIONS)
            nav_system = "\n\n".join(parts)
        nav_user = _render_prompt(
            "pi0_nav_pick_user.md",
            pi0_instruction=json.dumps(str(pi0_instruction)),
        )
        if enable_stage3_press:
            nav_user = "\n\n".join(
                [nav_user, _load_prompt("pi0_nav_pick_press_resume_user.md")]
            )
        return {
            "behavior_system_instructions": nav_system,
            "behavior_user_instructions": nav_user,
        }
