...
def _is_verified_terminal_tool_result(tool_result: Any) -> bool:
    """Trust only a toolkit-produced terminal result with an explicit payload."""

    result = getattr(tool_result, "result", None)
    return bool(
        getattr(tool_result, "is_finish", False)
        and isinstance(result, dict)
        and result.get("_finish") is True
    )


class _ToolCallAdmission:
    """Serialize MCP admissions and close the gate on the first terminal result."""
    ...
    def execute(self, name: str, arguments: dict[str, Any]) -> Any:
        with self._lock:
            # A queued or late MCP request is not a new toolkit/environment
            # admission. Replaying the original object also preserves the
            # validated terminal payload and receipt for the active turn.
            if self._terminal_result is not None:
                return self._terminal_result

            tool_result = self._toolkit.execute_tool(name, arguments)
            if _is_verified_terminal_tool_result(tool_result):
                self._terminal_result = tool_result
            if self._on_tool_result is not None:
                try:
                    self._on_tool_result(tool_result)
                except Exception:
                    # Planner-side interruption must never replace or corrupt
                    # the terminal tool response already produced by Toolkit.
                    logger.exception("MCP tool-result callback failed")
            return tool_result
    ...
    *,
    ...
    async def _call_tool(name: str, arguments: dict[str, Any]) -> types.CallToolResult:
        ...
        tr = await asyncio.get_running_loop().run_in_executor(
            None, tool_admission.execute, lookup, arguments or {}
        )
...
class HttpMcpServer:
    ...
    def __init__(
        ...
        *,
        ...
        self._tool_admission = _ToolCallAdmission(
            toolkit,
            on_tool_result=on_tool_result,
        )
    ...
    def start(self, *, ready_timeout_s: float = 30.0) -> str:
        ...
        app = _build_asgi_app(
            self._toolkit,
            admission=self._tool_admission,
        )
