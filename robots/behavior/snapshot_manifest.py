...
def write_snapshot_manifest(
    snapshot_path: str | Path, payload: dict[str, Any]
) -> Path:
...
def validate_snapshot_manifest(
    ...
    *,
    ...
    if not manifest_path.is_file():
        raise RuntimeError(
            f"simulator restore manifest is missing: {manifest_path}"
        )
    try:
    ...
    if not isinstance(template, dict):
        raise RuntimeError("simulator restore manifest lacks bootstrap template binding")
