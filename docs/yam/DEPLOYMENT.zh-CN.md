...
可选 `joint_servo: {enabled: true}` 在 `move_to` 的 IK 路径结束后启用有界关节 PI
到位检查，默认关闭。以固定 IK 关节目标减实测值积分，底层原有 PD 参数保持；默认外层
`kp=0`、`ki=0.5`、周期 0.1 s、最长 3 s，每关节累计偏置不超过 0.025 rad，
每轮偏置变化不超过 0.002 rad。修正仍经过同一服务的硬限位、步幅、跟踪和碰撞检查。
现场可将 `timeout_s` 显式延长至 8 s 观察慢速收敛；默认仍为 3 s，补偿与无进展停止上限不变。
连续三次同时满足关节误差 0.008 rad、末端位置误差 5 mm、姿态误差 0.05 rad 才到位。
超时、无进展、故障或取消时停止并请求保持；不自动增大偏置，不把路径发送完当成到位。
RPC 自身仍有传输超时，3 s 是循环期限，不能抢占一个已经阻塞的 RPC 调用。
配置由环境服务发布，Agent 不能通过工具参数调节 PI 增益或保护上限。
停止仍切换到实测位置保持并清除运动补偿。同回合重复停止不重新采集保持目标，
避免将后续下沉累积到目标中；这不消除底层静态保持误差。
PI 使用服务端声明的 `compact_control` 接口时，只读取关节状态并下发单步修正，
不读取或传输相机图像；与普通动作复用同一执行核心及全部保护，视觉检查仍在动作前后进行。
首次控制状态读取仍可能初始化硬件。旧服务没有此能力时沿用完整观测接口。

`control_diagnostics=true` 可在 observe 的 info 中读取已连接 SDK 的实际排队目标、
