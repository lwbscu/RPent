...
## 2026-09-10 J3 单关节隔离诊断

用户明确继续采用 RPent／主从遥操参数，并批准单关节验证。诊断脚本经同一环境
`control_step`、路径保护和 RLinf runtime 执行，没有经过 IK；不是 Agent 整理任务验收。
以驱动实际保持目标固定其余关节和夹爪，J3 目标设为实测值加 0.015 rad。
新回合只做 bookkeeping，同步环境步幅基准，并验证 SDK 目标未改变、每条指令未被截断。

首次 PD 观察 3 s，误差仍为 0.015 rad；之后仅 J3 积分，约 2.18 s 因 1 s 无进展停止，
偏置 0.01277 rad，J3 实际仅前进约 0.00038 rad，未到位。
第二次只将隔离脚本的无进展观察窗口延至 3 s，其余限幅不变；J3 偏置达到 0.025 rad，
实际前进约 0.00381 rad，但固定目标的 J4 实测变化约 -0.01030 rad，
触发“非 J3 关节偏移超过 0.01 rad”诊断保护并停止，不能判为单关节到位。
证据在本机 `logs/yambox_deployment/j3_isolation_20260910/` 与 `j3_isolation_progress3_20260910/`。

新增只读电机缓存诊断并重启自有服务后，左 J3 MOS/转子为 42/43°C，J4 为 47/49°C，
两臂所有电机的缓存状态码均为 1（SDK 已使能）。这是重启后的快照，不是动作期间的温度／故障历史，
也不能排除内部限流。没有改动电机参数、增加补偿幅度或搬动物体。
新增缓存诊断及接口相关 44 项回归通过；完整 YAM 回归中原有测试桩参数不匹配已修正并复测。

## 2026-09-10 后续 5 mm 跟踪诊断

用户确认现场无可见托挡或线缆牵拉。已加载 `5d8f0a8` 的停止修复，经 RPent
执行左臂 5 mm 上抬：3 s 内 J3 实测仅转约 0.00267 rad，要求约 0.01893 rad，
末端误差 7.83 mm，超时停止。停止时间戳在 Agent 退出后保持不变，重复停止未重设目标。

随后仅将 PI 总观察上界改为 8 s（默认仍 3 s），其余电机参数、偏置限幅和 1 s 无进展停止不变。
从重启后实测状态重新规划另一次 5 mm 上抬，约 4.32 s 时无进展停止：
J3 偏置达到 0.025 rad，末端误差 7.89 mm，最大关节误差 0.01283 rad。
没有物体搬运，不能标记为成功。证据分别在本机日志 `task_a_resume_20260910/`、`settling8_20260910/`。
观察时限改动的相关 67 项离线测试通过。

已核对参数来源：RPent 与 `collect_tabletop_cleanup_1.sh` 的主从数采配置一致，
重力系数 `[1,1.1,1.1,1.2,1,1]`、摩擦补偿关闭；pico VR 配置则为
`[1.3,1.3,1.3,1.3,1,1]`、摩擦补偿开启。相同 SDK 的位置控制 kp/kd 相同。
这是一项实际配置差异，尚不能单独证明是跟踪失败根因。VR 参数候选只写入本机配置文件，未加载。
