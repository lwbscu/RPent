...
def test_toolkit_modes_expose_exactly_one_closed_control_surface(tmp_path):
    ...
    nav_pick_stage3 = BehaviorToolkit(
        control_mode=PI0_NAV_PICK_VLA_MODE,
        primitives_kwargs=primitives_kwargs,
        enable_stage3_press=True,
    )
    assert [spec["name"] for spec in nav_pick_stage3.get_tools_spec()] == [
        "pi0_nav_pick",
        *POST_PICK_TOOL_NAMES,
        *STAGE3_PRESS_TOOL_NAMES,
    ]
...
def test_nav_pick_stage2_surface_uses_existing_names_and_bound_hand_schemas():
    ...
    checkpoint_name = save_schema["properties"]["checkpoint_name"]
    assert checkpoint_name["oneOf"][0]["const"] == "state_checkpoint_2"
    assert "tmp_state_checkpoint_" in checkpoint_name["oneOf"][1]["pattern"]
    assert save_schema["properties"]["stage"]["enum"] == [
        "pre_press_alignment",
        "temporary_restore_point",
    ]
    ...
    restore_name = restore_schema["properties"]["checkpoint_name"]
    assert restore_name["oneOf"][0]["enum"] == [
        "state_checkpoint_1",
        "state_checkpoint_2",
    ]
    assert "tmp_state_checkpoint_" in restore_name["oneOf"][1]["pattern"]
