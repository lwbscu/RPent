...
from rpent.utils.redaction import redact_value
...
def test_stage3_env_rpc_requires_machine_side_opt_in():
    class Env:
        _control_mode = PI0_NAV_PICK_VLA_MODE

        def __init__(self, enabled):
            self._stage3_press_enabled = enabled

        @staticmethod
        def inspect_toggle_geometry():
            return "geometry"
    ...
    disabled = _MainThreadDispatcher(Env(False), threading.Event())
    for method in (
        "env.inspect_toggle_geometry",
        "env.post_pick_close_press_gripper",
    ):
        with pytest.raises(ValueError, match="stage-3 RPC is not enabled"):
            disabled._dispatch(method, (), {})

    enabled = _MainThreadDispatcher(Env(True), threading.Event())
    assert enabled._dispatch("env.inspect_toggle_geometry", (), {}) == "geometry"
    assert enabled._dispatch("env.post_pick_close_press_gripper", (), {}) == "closed"
...
def test_stage3_flag_is_valid_only_for_radio_nav_pick_mode(tmp_path):
    provider, parser, invalid = _provider_args(
        tmp_path,
        "--behavior-control-mode",
        "planner_tools",
    ...
    with pytest.raises(SystemExit):
        provider.validate_args(invalid, parser)

    checkpoint = tmp_path / "checkpoint"
    _write_checkpoint(checkpoint)
    valid_root = tmp_path / "valid"
    valid_root.mkdir()
    provider, parser, valid = _provider_args(
        valid_root,
        "--behavior-control-mode",
        PI0_NAV_PICK_VLA_MODE,
        "--behavior-stage3-press",
    ...
    provider.validate_args(valid, parser)
    prompt_vars = provider.prompt_vars(
        valid,
        output_dir=valid_root / "stage3-run",
        recipe_tag=provider.recipe_tag(valid),
    )

    assert prompt_vars["behavior_stage3_press"] is True
    assert (
        "Post-prepress button-press phase"
        in prompt_vars["behavior_system_instructions"]
    )
...
def test_run_manifest_full_records_redacted_owned_process(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(behavior_manifest, "_proc_identity", lambda pid: (pid, 123456))
    ...
    assert process["sid"] == 81234 and process["start_ticks"] == 123456
...
def test_redaction_removes_url_userinfo_and_nested_credentials():
    command = redact_command(
        [
            "tool",
            "--base-url",
            "https://private-user:private-secret@example.test/v1",
            "--api-key",
            "key-secret",
        ]
    )
    serialized = json.dumps(
        redact_value(
            {
                "error": "token=token-secret at https://user:pass@example.test/v1",
                "authorization_token": "nested-secret",
    ...
    assert "raw-bearer-secret" not in behavior_manifest.redact_text(
...
@pytest.mark.parametrize(
    "raw,secret",
    [
        ("Proxy-Authorization: Basic cHJveHktc2VjcmV0", "cHJveHktc2VjcmV0"),
        ("Bearer bare-bearer-secret", "bare-bearer-secret"),
        ("Basic YmFzaWMtc2VjcmV0", "YmFzaWMtc2VjcmV0"),
    ],
)
def test_redaction_covers_proxy_and_bare_auth_schemes(raw, secret):
    assert secret not in behavior_manifest.redact_text(raw)


def test_claude_sdk_stderr_callback_redacts_before_logging(monkeypatch, tmp_path):
    captured = []
    ...
    monkeypatch.setattr(
        "rpent.cerebrum.claude_code.logger.debug",
        lambda message, value: captured.append(message % value),
    )
    cerebrum = ClaudeCodeCerebrum(
        output_dir=str(tmp_path), repo_root=tmp_path, tool_only=True
    )
    options = cerebrum._build_options(SDK, toolkit=Toolkit(), max_turns=1)

    options.stderr("Authorization: Bearer claude-stderr-secret")

    assert "claude-stderr-secret" not in captured[0]
...
def test_behavior_spawn_registration_defers_termination_signals(monkeypatch):
    calls = []
    handlers = {
    ...
    monkeypatch.setattr(behavior_runtime.signal, "getsignal", getsignal)
    monkeypatch.setattr(behavior_runtime.signal, "signal", set_signal)

    with behavior_runtime._defer_termination_signals():
        calls.append(("inside", set()))

    assert calls[0][0] == signal.SIGINT
    assert calls[1][0] == signal.SIGTERM
    assert calls[2] == ("inside", set())
    assert calls[3] == (signal.SIGINT, signal.default_int_handler)
    assert calls[4] == (signal.SIGTERM, signal.SIG_DFL)

    registered = []
    with pytest.raises(SystemExit) as raised:
        with behavior_runtime._defer_termination_signals():
            handlers[signal.SIGTERM](signal.SIGTERM, None)
            registered.append("manifest-written")
    assert raised.value.code == 128 + signal.SIGTERM
    assert registered == ["manifest-written"]
...
def test_stage3_runtime_closes_and_deletes_only_temporary_checkpoint_json(
    monkeypatch, tmp_path
):
    class Toolkit:
        control_mode = PI0_NAV_PICK_VLA_MODE
        enable_stage3_press = True
        ...
        _primitives = type(
            ...
            (),
            {
                "last_result": {
                    "primitive_success": False,
                    "task_success": False,
                    "stop_reason": "bounded_press_failed",
                }
            },
        )()
    ...
    persistent1 = checkpoint_dir / "state_checkpoint_1.json"
    persistent2 = checkpoint_dir / "state_checkpoint_2.json"
    temporary = checkpoint_dir / "tmp_state_checkpoint_before_press.json"
    temporary_partial = checkpoint_dir / "tmp_state_checkpoint_partial.json.tmp"
    for path in (persistent1, persistent2, temporary, temporary_partial):
    ...
    assert waited == []
    assert persistent1.is_file()
    assert persistent2.is_file()
    assert not temporary.exists()
    assert not temporary_partial.exists()
