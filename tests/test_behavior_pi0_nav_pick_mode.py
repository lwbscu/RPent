...
def test_robot_motion_checkpoint_payload_is_json_allowlisted_without_sim_state(
    ...
    public_saved = BehaviorPrimitives(
        env=facade, output_dir=tmp_path
    ...
    assert public_saved["primitive_success"] is True
    assert public_saved["task_success"] is False
    assert public_saved["official_success_source"] == 'info["done"]["success"]'
